...
3. 新增/变更的文件：解析 → 分块 → Embedding → 写入 Qdrant
4. 更新 processed_cache 记录
...
from chunk_strategy import chunk_all, Chunk, chunks_to_json
...
class ProcessedCache:
    ...
    def get_file_record(self, file_path: str) -> Dict[str, Any]:
        """获取单个文件的缓存记录"""
        return self.data.get(file_path, {})

    def set_file_record(self, file_path: str, record: Dict[str, Any]):
        """设置单个文件的缓存记录"""
        self.data[file_path] = record

    def is_file_unchanged(self, file_path: Path, file_hash: str) -> bool:
        """判断文件是否未变更"""
        record = self.get_file_record(str(file_path))
...
def process_file(file_path: Path, vector_store: VectorStore) -> Dict[str, Any]:
    ...
    # 3. 分块
    from chunk_strategy import chunk_document
...
def run_ingest(force: bool = False, reset: bool = False):
    ...
    for fp in file_paths:
        ...
        # 判断是否跳过
        if not force and cache.is_file_unchanged(fp, file_hash):
        ...
        try:
            ...
            cache.set_file_record(str(fp), record)
            stats["processed"] += 1
            stats["total_chunks"] += record.get("chunk_count", 0)

            if record["status"] == "empty":
                stats["failed"] += 1  # 空文件也算异常

        except Exception as e:
            logger.error(f"[Ingest] 处理失败 {fp.name}: {e}")
            cache.set_file_record(str(fp), {
                "file_hash": file_hash,
                "status": "failed",
                "error": str(e),
            })
